...
if(getisset("add")) {
    ...
    try {
        WeldingConsumable::create($post);
        bilgi("WeldingConsumable has been created","success");
    } catch (\Throwable $th) {
        dump($th);
    }
...
if(getisset("delete")) {
    WeldingConsumable::where('id', get('delete'))->delete();

}

if(getisset("delete-all")) {
    WeldingConsumable::truncate();
}


$tableColumns = table_columns("welding_consumables");
        ?>

<?php  
$tableName = "welding_consumables"; ?>
...
<table class="table table-bordered table-sm" style="table-layout:fixed;width:200%">
    ...
    <tbody>
        <?php 
        foreach(WeldingConsumable::all() AS $currentType) {
                ?>
            <tr>
            <?php foreach($tableColumns AS $column)  {  ?>
                <?php if($column=="id")  {  ?>
