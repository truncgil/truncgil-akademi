...
if(getisset("add")) {
    ...
    try {
        JobDescription::create($post);
        bilgi("JobDescription has been created","success");
    } catch (\Throwable $th) {
        dump($th);
    }
...
if(getisset("delete")) {
    JobDescription::where('id', get('delete'))->delete();
...
<table class="table table-bordered table-sm" style="table-layout:fixed;width:100%">
    ...
    <tbody>
        <?php 
        foreach(JobDescription::all() AS $workType) {
                ?>
            <tr>
            <?php foreach($tableColumns AS $column)  {  ?>
                <?php if($column=="id")  {  ?>
