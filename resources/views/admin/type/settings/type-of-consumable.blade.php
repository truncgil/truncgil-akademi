...
if(getisset("add")) {
    ...
    try {
        TypeOfConsumable::create($post);
        bilgi("TypeOfConsumable has been created","success");
    } catch (\Throwable $th) {
        dump($th);
    }
...
if(getisset("delete")) {
    TypeOfConsumable::where('id', get('delete'))->delete();

}

if(getisset("delete-all")) {
    TypeOfConsumable::truncate();
}

$tableName = "type_of_consumables"; 
...
<table class="table table-bordered table-sm" style="table-layout:fixed;width:100%">
    ...
    <tbody>
        <?php 
        foreach(TypeOfConsumable::all() AS $workType) {
                ?>
            <tr>
            <?php foreach($tableColumns AS $column)  {  ?>
                <?php if($column=="id")  {  ?>
