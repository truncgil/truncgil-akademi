...
if(getisset("add")) {
    ...
    try {
        PerformedWorkType::create($post);
        bilgi("PerformedWorkType has been created","success");
    } catch (\Throwable $th) {
        dump($th);
    }
...
if(getisset("delete")) {
    PerformedWorkType::where('id', get('delete'))->delete();

}

if(getisset("delete-all")) {
    PerformedWorkType::truncate();
}

$tableName = "performed_work_types"; 
...
<table class="table table-bordered table-sm" style="table-layout:fixed;width:100%">
    ...
    <tbody>
        <?php 
        foreach(PerformedWorkType::all() AS $workType) {
                ?>
            <tr>
            <?php foreach($tableColumns AS $column)  {  ?>
                <?php if($column=="id")  {  ?>
