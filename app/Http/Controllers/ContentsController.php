...
class ContentsController extends Controller
{
    ...
    public function default(Request $request, string $slug,string $lang="tr")
    {
		...
		if(is_null($c)) {
			$c = db("articles")->where("slug", $slug)->first();
			if($c) {
				$issue = db("issues")->where("slug",$c->kid)->first();
				$volume = db("volumes")->where("slug",$issue->kid)->first();
				$volume->other = json_decode($volume->json);
				$issue->other = json_decode($issue->json);
				$c->issue = $issue;
				$c->volume = $volume;
				$c->other = json_decode($c->json);
			//	array_merge($c,json_decode($c->json));
			//	dd($c);
				$c->keywords = explode(",",$c->other->keywords);
				$type = "Article";
			}
			
		}
		if(is_null($c)) {
			$c = db("volumes")->where("slug", $slug)->first();
			$type = "Volume";
		}
		if(is_null($c)) {
			$c = db("issues")->where("slug", $slug)->first();
			if($c) {
				$volume = db("volumes")->where("slug",$c->kid)->first();
				$c->volume = $volume;
				$type = "Issue";
			}			
